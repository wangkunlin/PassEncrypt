...
class GoogleServicesTask extends DefaultTask {
    ...
    void action() throws IOException {
        if (!quickstartFile.isFile()) {
            throw new GradleException("File ${quickstartFile.getName()} is missing. "
                    + "The Google Services Plugin cannot function without it. Searched Location: ${searchedLocation}")
        }
        project.logger.warn("google-services Plugin based on com.google.gms:google-services:4.2.0")
        project.logger.warn("Parsing json file: ${quickstartFile.getPath()}")
    ...
    private static void handleFirebaseUrl(JsonObject rootObject, Map<String, String> resValues)
            throws IOException {
     ...
     * @throws IOException
     */
    private static void handleProjectNumberAndProjectId(JsonObject rootObject, Map<String, String> resValues)
            throws IOException {
        JsonObject projectInfo = rootObject.getAsJsonObject("project_info")
        if (projectInfo == null) {
            throw new GradleException("Missing project_info object")
    ...
    private static void handleWebClientId(JsonObject clientObject, Map<String, String> resValues) {
    ...
    private static void handleGoogleApiKey(JsonObject clientObject, Map<String, String> resValues) {
        String apiKey = getAndroidApiKey(clientObject)
        if (apiKey != null) {
    ...
    private static String getAndroidApiKey(JsonObject clientObject) {
    ...
    private JsonObject getClientForPackageName(JsonObject jsonObject) {
        ...
        if (array != null) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (getPackageName() == clientPackageName.getAsString()) {
                    return clientObject
                }
            }
        }
    ...
    private static void handleGoogleAppId(JsonObject clientObject, Map<String, String> resValues)
        ...
        if (GOOGLE_APP_ID_VERSION != version) {
            throw new GradleException(
